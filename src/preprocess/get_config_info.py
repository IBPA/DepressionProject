...
df_var_info = pd.read_csv(DEFAULT_VARIABLE_INFO, dtype='str', encoding= 'unicode_escape')
#df_var_info = df_var_info.applymap(lambda x: x.strip())  # remove white space
print(f"Categorical Info is null: {df_var_info[df_var_info['Categorical'].isnull()]}")
categorical = df_var_info.loc[df_var_info['Categorical'] == '1', 'RelabeledName'].tolist()
...
print(f"Mental Health is null: {df_var_info[df_var_info['Child Mental Health Variable (1=Yes, 0 =No) '].isnull()]}")
